...
def _deterministic_prewarm_params(
    ...
    """Return legacy policy fields for the v6 sharded-prewarm RPC.

    ``HybridSearchActor.prewarm_partitions_deterministic`` keeps the old
    ``policy`` / ``ram_bytes`` parameters so older driver call sites and
    result records stay readable, but the Lance 7.0 path ignores both
    and delegates placement to ``dataset.prewarm_index(...,
    partition_ids=...)``.
    """
    if scenario == "moka":
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default=1.0,
        help=(
            "Legacy no-op. Lance 7.0 sharded prewarm uses "
            "dataset.prewarm_index(name, partition_ids=...) and delegates "
            "placement to the distributed cache, so there is no "
            "operator-visible L1 fraction to scale. Accepted for backward "
            "compatibility; values other than 1.0 are flagged as a no-op."
        ),
    )
    p.add_argument(
...
def _run_invalidation_drill(
    *,
    ...
    """Run the optional --simulate-invalidation drill.
    ...
    if args.prewarm == "forced":
        ...
        if args.scenario == "no-cache":
            ...
            if args.scenario == "distributed" and args.prewarm_ram_fraction != 1.0:
                # Lance 7.0 prewarm delegates placement to the distributed
                # cache, so this legacy v4 knob has no effect.
                print(
                    f"[driver] --prewarm-ram-fraction={args.prewarm_ram_fraction} "
                    "ignored under --scenario distributed: "
                    "dataset.prewarm_index(partition_ids=...) does not expose "
                    "an L1 fraction to scale."
                )
            if args.scenario == "distributed":
                print(
                    "[driver] sharded prewarm via "
                    "dataset.prewarm_index(partition_ids=...) "
                    f"(legacy policy field={policy!r}) — partition assignment "
                    f"(round-robin mod {args.num_actors}) over "

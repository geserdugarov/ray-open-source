...
* Adds `--prewarm sharded` which assigns disjoint partition slices to
  ...
  placement policy picked from `--scenario`: `hybrid_tiered` fills
  foyer L1 up to its per-actor budget and forces the rest to L2;
  `moka_ram_cap` loads until `--dram-gb` is full and stops; `no-cache`
  is a no-op that just registers ownership. Per-actor prewarm cost
  stays flat as `--num-actors` grows, but per-query recall becomes
...
def _deterministic_prewarm_params(
    scenario: str,
    dram_bytes: int,
    codecless_bytes: int | None,
    hybrid_ram_fraction: float = 1.0,
) -> tuple[str, int]:
    """Map (scenario, per-actor DRAM split) → (policy, ram_bytes).

    ``ram_bytes`` is the per-actor DRAM budget the deterministic prewarm
    uses for the fill/stop decision:

    * moka — full per-actor DRAM (Moka has no L2 spill).
    * hybrid — foyer L1 budget only. With ``--codecless-mb`` set this is
      ``dram_bytes - codecless_bytes`` (advanced); otherwise Lance's
      ``with_hybrid_cache`` 90/10 split applies and foyer L1 is
      ``dram_bytes - dram_bytes // 10`` (~90%). The codec-less Moka
      tier serves codec-less metadata (top-level vector index objects,
      scalar index pages, legacy IVF v1 entries) and is filled
      implicitly when each worker opens the index — it is not the
      prewarm target.

    ``hybrid_ram_fraction`` scales the hybrid foyer L1 target. Default
    ``1.0`` requests the nominal full L1 capacity; foyer's sharded
    S3FIFO may spill a subset of L1 admissions to L2 under per-shard
    overflow (surfaced in stats as ``disk_bytes_unknown_spills``).
    Values below ``1.0`` trade nominal L1 fill for guaranteed-no-spill
    residency on heavily sharded backings — Lance's docs suggest
    roughly ``0.5`` for full-L1 residency.
    """
    if scenario == "moka":
        return "moka_ram_cap", int(dram_bytes)
    if scenario == "hybrid":
        if codecless_bytes is None:
            # Match lance Session::with_hybrid_cache's 90/10 split:
            # codec-less Moka gets dram_bytes // 10, foyer L1 gets the
            # rest. See lance rust/lance/src/session.rs for the source
            # of truth.
            foyer_l1_bytes = dram_bytes - dram_bytes // 10
        else:
            foyer_l1_bytes = dram_bytes - codecless_bytes
        return "hybrid_tiered", int(foyer_l1_bytes * hybrid_ram_fraction)
    raise ValueError(
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help=(
            ...
            "--scenario hybrid (fill foyer L1 up to its budget, push the "
            "remainder to L2), moka_ram_cap for --scenario moka (load "
            "until --dram-gb is full, then stop), no-op for --scenario "
            "no-cache (registers ownership only). The measure phase uses "
            "search_partitions over each actor's owned slice "
            "(per-query partial recall — see README); "
    ...
    p.add_argument(
        ...
        default=1.0,
        help=(
            "Scales the per-actor foyer L1 budget passed to hybrid_tiered "
            "deterministic prewarm. Default 1.0 requests the nominal full "
            "L1 capacity; foyer's sharded S3FIFO may then spill a subset of "
            "L1 admissions to L2 under per-shard overflow (surfaced as "
            "spilled=<count> / disk_bytes_unknown_spills in stats). Set "
            "below 1.0 to trade nominal fill for strict L1 residency — "
            "Lance suggests ~0.5 for deterministic full-L1 residency on "
            "heavily sharded backings. Ignored for --scenario moka "
            "(no L2 spill) and --scenario no-cache."
        ),
    )
    p.add_argument(
        ...
        help=(
            ...
            "flag is off the printed residency-shift comparison is "
            "skipped because there is no pre-measure snapshot."
        ),
    )
...
def main() -> int:
    ...
    if args.prewarm == "forced":
        ...
        if args.scenario == "no-cache":
            ...
            policy, ram_bytes_budget = _deterministic_prewarm_params(
                ...
                dram_bytes,
                codecless_bytes,
                hybrid_ram_fraction=args.prewarm_ram_fraction,
            )
            print(
            ...
            for r in prewarm_results:
                ...
                # spilled counts partitions foyer's sharded S3FIFO evicted
                # from L1 to L2 during this call under per-shard overflow.
                # Already included in `loaded_to_disk`; surfaced separately
                # so users can tell shard skew from intentional disk
                # placement when tuning --dram-gb / --prewarm-ram-fraction.
                spilled = ps.get("disk_bytes_unknown_spills", "?")
                print(
                    f"  actor={r['actor_id']} prewarm={r['duration_s']:.1f}s "
                    f"owned={r['n_partitions']} "
                    f"ram={ps.get('loaded_to_ram', '?')} "
                    f"disk={ps.get('loaded_to_disk', '?')} "
                    f"spilled={spilled} "
                    f"skipped={ps.get('skipped_existing', '?')} "
            ...
            # Per-actor L2 directory footprint — observability only.
            # The actor reports its local NVMe so the snapshot works in
            # real-cluster mode where the L2 dir lives on the actor node.
            if args.scenario == "hybrid":
                actor_l2_dirs = [
    ...
    # snapshots exist the comparison below shows query-driven L1↔L2
    # churn: any partition that moved out of RAM during measure appears
    # in this snapshot's ``not_in_ram`` but the previous one's
    # ``in_ram``. With foyer's WriteOnEviction policy the displaced
    # entry typically lands in L2, so the comparison surfaces query-
    # driven movement even though per-partition L2 residency is not
    # yet directly observable through pylance. Runs unconditionally for
    # sharded non-no-cache scenarios because it runs *after* measure
    # and so cannot pollute the measured workload — unlike the
    # pre-measure probe which is opt-in. Done before actor close so the
    # cache is still in its post-measure state.
    ...
    if do_post_residency_probe:
        ...
        # that were DRAM-resident post-prewarm but not post-measure were
        # evicted by query traffic (foyer's WriteOnEviction policy
        # demotes them to L2). Promotions are the reverse: a partition
        # that was not RAM-resident at the post-prewarm probe but is now,
        # which on hybrid sessions implies an L2 → L1 promotion driven
        # by the query path.
        if pre_measure_residency:
            pre_by_id = {r["actor_id"]: set(r["in_ram"]) for r in pre_measure_residency}
            print("\n=== Residency shift (post-prewarm → post-measure) ===")
            for r in post_measure_residency:
                aid = r["actor_id"]
                pre_set = pre_by_id.get(aid, set())
                post_set = set(r["in_ram"])
                stayed = len(pre_set & post_set)
                evicted = len(pre_set - post_set)
                promoted = len(post_set - pre_set)
                print(
                    f"  actor-{aid:<3} stayed_in_ram={stayed:<5} "
                    f"evicted={evicted:<5} promoted_into_ram={promoted:<5}"
                )

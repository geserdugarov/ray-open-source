...
class HybridSearchActor:
    ...
    def prewarm_index(self, index_name: str) -> Dict[str, Any]:
        ...
        With a hybrid Session this populates both the foyer L1 (DRAM)
        and L2 (NVMe) tiers; subsequent queries hit cache. Returns post-
        prewarm stats so the driver can verify the cache actually filled.
        """
    ...
    def prewarm_partitions_deterministic(
        ...
        * ``hybrid_tiered`` — fill foyer L1 up to ``ram_bytes``, then
          force the remaining partitions into L2 deterministically.
          Requires a hybrid Session.
        ...
        * ``normal`` — existing behavior (every requested partition runs
        ...
        Returns the per-policy prewarm counters (``loaded_to_ram``,
        ``loaded_to_disk``, ``stopped_before``, ``ram_bytes_deep_size``,
        ``disk_bytes_serialized``) so the driver can verify the cache
        actually filled in the expected shape.
        """
        if not hasattr(self._ds, "prewarm_vector_cache"):
            raise RuntimeError(
                "this pylance build does not expose dataset.prewarm_vector_cache; "
    ...
    def check_partition_residency(
        ...
        """Probe each owned partition for DRAM residency without side effects.

        Lance does not (yet) expose ``IvfIndexSearcher::partition_is_cached``
        and ``::partition_is_in_l2`` through the Python bindings, but the
        existing ``prewarm_vector_cache`` API with
        ``policy='moka_ram_cap', ram_bytes=0`` doubles as a no-load probe:
        pass 1 counts every DRAM-resident partition in ``skipped_existing``,
        pass 2 short-circuits on ``ram_bytes_deep_size >= ram_bytes`` (i.e.
        ``0 >= 0``) and never loads anything. Single-partition probes
        therefore tell us, for each id, whether it is currently in the
        foyer L1 (DRAM) tier.

        L2 residency has no equivalent no-load probe in current pylance,
        so the report includes the L2 directory footprint (file count and
        on-disk bytes) and the session-level ``index_cache_stats`` for
        cross-reference against the prewarm-time ``loaded_to_disk`` count.
        Use ``not_in_ram = owned - in_ram`` as the proxy for "either in L2
        or evicted" — comparing it to the prewarm-time ``loaded_to_disk``
        count tells you whether queries promoted L2 partitions into RAM
        or evicted RAM-resident partitions back to L2.
    ...
    def close(self) -> None:
        """Flush foyer DRAM → NVMe before the actor exits. Required for
        warm restart. Idempotent for default (Moka) sessions."""
        self._sess.close()

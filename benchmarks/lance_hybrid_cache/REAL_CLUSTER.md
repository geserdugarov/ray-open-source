...
raises a clear `RuntimeError` on first use, so verify those bindings
before starting an end-to-end sharded run.

The 2.54 port keeps the runnable distributed-cache benchmark and its
cluster operating notes in this directory. Historical plan documents
from the older 2.53 branch were intentionally not replayed.

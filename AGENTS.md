...
`master-described` is aligned with `master` at `9ed00cca01`, after `master` was rebased from `cf7ae60796`. The branch adds repository guidance and the high-level summaries under `plans/docs/`; keep those summaries synchronized when rebasing across `master` changes.
...
- `.claude/` and `doc/.claude/` - Claude Code guidance, rules, and skills
...
ci/env/install-bazel.sh  # installs Bazelisk; Bazel 7.5.0 is supported
...
Documentation-specific guidance lives in `doc/.claude/CLAUDE.md`. Claude Code skills include `rst-to-myst` and `sphinx-fix`.
...
- Prefer source-of-truth docs such as `doc/source/ray-contribute/development.md` over stale command snippets.

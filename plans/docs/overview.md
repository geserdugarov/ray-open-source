...
```bash
pip install -U <nightly-wheel-url>
...
Primary CI is **Buildkite** (`.buildkite/*.rayci.yml`). Tests are conditionally run based on changed files (`ci/pipeline/determine_tests_to_run.py`). GitHub Actions handle lightweight checks only.

## Documentation

Built with Sphinx from `doc/source/`. Covers user guides, API references, tutorials, and developer documentation for all modules. Build: `make -C doc/ html`.

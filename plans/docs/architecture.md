...
Snapshot: this described branch is based on `upstream/releases/2.54.0` at `1ea4980a1d`, after the `ray-2.54.0` tag at `48bd1f8fa4`.
...
|   +-- monitors/           CPU and memory monitor implementations
|
...
Python shared internals now include `python/ray/_common/` for cross-library non-public utilities. Library code should prefer `_common` over reaching into `python/ray/_private/`.

...
- `CompiledDAG` (`compiled_dag_node.py`, 144 KB) — Compiled executable graph
...
**Location:** `AGENTS.md` (`CLAUDE.md` symlink)

Repository-level guidance for coding agents. It summarizes build, test, lint, documentation, and architecture conventions for the current `master-described` branch.

### Claude Code Guidance and Skills

**Location:** `.claude/`

Version-controlled Claude Code configuration:
- `CLAUDE.md` — Short repository guide that points to source-of-truth docs and skills
- `rules/` — Scoped coding rules, including Python style and C++ time/clock usage
- `skills/` — Rebuild, lint, Buildkite log fetching, and Ray dependency management skills

### Documentation Agent Guidance

**Location:** `doc/.claude/`

Documentation-specific agent guidance:
- New doc pages under `doc/source/` should be MyST Markdown (`.md`)
- Docs-only PRs should stay within paths mapped to the `doc` CI tag
- Cross-links from `.md` to `.rst` sources should use the `{doc}` role
- Skills include `rst-to-myst` and `sphinx-fix`
